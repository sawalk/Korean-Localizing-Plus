...
function mod:onRender()
    if currentPillSprite then
        ...
        if mod:isJacobAndEsau(player) then
            local offsetX = 110 -- Left top X offset
            local offsetY = 44 -- Left top Y offset
            renderPosition = Vector(offsetX, offsetY)
        else
            local offsetX = -140.5 -- Bottom right X offset
            local offsetY = -25.5 -- Bottom right Y offset
